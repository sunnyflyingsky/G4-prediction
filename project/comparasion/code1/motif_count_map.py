...
def map(inpath,refpath,outpath):
    ...
    with open(refpath) as read_object:
        for line in read_object:
            ...
            try:gene6k[info[0]].append([info[1],info[2]])
            except:gene6k[info[0]]=[[info[1],info[2]]]
    ...
    plt.title('motif hits in G4 peak summits',loc='center',fontsize=36,fontweight='black')
    plt.xlabel('distance to G4 peak summits',fontsize=24)
    plt.ylabel('motif count',fontsize=24)
    plt.plot(range(len(res)),res,'r',label='G4 profile')
    plt.xticks([0,3000,6000],labels=['-3kb','summits','3kb'],fontsize=24)
...
if __name__=='__main__':
    ...
    refpath = 'data/source_data/K562_1a.bed'
    outpath = 'comparasion/G4_third/res/hg19_G4_motif_count_profile5'
