...
def generate_subscript():
    ...
    if not os.path.isfile('tem/tem.py'):
        ...
        SubScriptsFHD.write("            rfhds[i].write('\\t'.join(['0' if value == 'n/a' else value for value in output]) + '\\n')\n")
        SubScriptsFHD.write("        else:\n")
        SubScriptsFHD.write("            rfhds[i].write('\\t'.join(['0' for i in range(datapoints)]) + '\\n')\n")
        SubScriptsFHD.write("\n")
